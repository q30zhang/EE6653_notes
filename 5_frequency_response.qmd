...
\begin{tikzpicture}[>=latex, thick]
    ...
    \draw[->] (-3,0) -- (sys.west);
    \draw[->] (sys.east) -- (3,0);
    \node[left] at (-1.2,0.3) {$x(t)=A \cdot \sin(\omega t+\theta)$};
    \node[right] at (1.2,0.3) {$y(t) = A \cdot \color{blue}{\underline{\color{black} \lvert G(j\omega) \rvert}} \color{black} \cdot \sin\Big(\omega t + \theta + \color{blue}{\underline{\color{black} \angle G(j\omega)}}\color{black}\Big)$};
\end{tikzpicture}
```
:::
